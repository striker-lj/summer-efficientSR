...
from basicsr.archs.rrdbnet_arch import RRDB, ResidualDenseBlock
from .arch_util import to_2tuple, trunc_normal_, make_layer
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop_rate=0.,
                 attn_type='vanilla', d_hid=32, f=8, Dattn_dropout=0.):
    ...
    def attn_init(self, ):
        if self.attn_type == 'vanilla':
        ...
        elif self.attn_type == 'vanilla_kout':
            self.q = nn.Linear(self.dim, self.dim, bias=self.qkv_bias)
        ...
        elif self.attn_type == 'dense_gelu':
            self.w_1 = nn.Linear(self.dim // self.num_heads, self.d_hid)  # 由原始input的维度d_k到中间的维度d_hid
            self.w_2 = nn.Linear(self.d_hid, 64)  # 由中间的维度d_hid到设置的最大维度N
            self.gelu = nn.GELU()
        ...
        elif self.attn_type == 'dense_mlp3':
            self.w_1 = nn.Linear(self.dim // self.num_heads, 64)  # ????input?????d_k???м????????d_hid1
            self.w_2 = nn.Linear(64, 64)  # ????input?????d_k???м?????d_hid
            self.w_3 = nn.Linear(64, 64)  # ????input?????d_k???м?????d_hid
            self.relu = nn.ReLU()
            ...
            self.f_a = nn.Linear(self.dim, 2 * self.num_heads)
            self.f_b = nn.Linear(self.dim, 2 * self.num_heads)
    ...
    def forward(self, x, mask=None):
        ...
        if self.attn_type == "vanilla":
            ...
            attn = self.w_2(self.gelu(self.w_1(q)))
            ...
            attn = self.w_3(self.relu(self.w_2(self.relu(self.w_1(q)))))
            ...
            a = self.f_a(x).reshape(b_, n, self.num_heads, 2).permute(0, 2, 1, 3)
            b = self.f_b(x).reshape(b_, n, self.num_heads, 2).permute(0, 2, 1, 3)


            h_a = torch.repeat_interleave(a, 4, -1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.SElayer = SELayer(dim, SE_reduction)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

        self.esr_body = make_layer(RRDB, num_block, num_feat=self.num_feat, num_grow_ch=num_grow_ch)
...
class SynSwinIR_esr(nn.Module):
    ...
    def __init__(self,
                 ...
                 SE_reduction=6,
                 **kwargs):
        super(SynSwinIR_esr, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
